...
try {
    $stmt = $pdo->query("
        SELECT d.id, d.type_dechet, d.quantite_kg
        FROM dechets_collectes d
        ORDER BY d.quantite_kg DESC
    ");

    $query = $pdo->prepare("SELECT * FROM dechets_collectes ");
    $query->execute();
...
<div class="flex h-screen">
    ...
    <div class="flex-1 p-8 overflow-y-auto">
        <!-- Titre -->
        <h1 class="text-4xl font-bold text-blue-800 mb-6">Liste des déchets récoltés</h1>
        ...
        <div class="overflow-hidden rounded-lg shadow-lg bg-white">
            <table class="w-full table-auto border-collapse">
                ...
                <tr>
                    ...
                    <th class="py-3 px-4 text-left">Quantité (kg)</th>
                </tr>
                </thead>
...
if ($dechets) {
    // Boucle sur chaque déchet
    foreach ($dechets as $dechet) {
        ?>
        <tr class="hover:bg-gray-100 transition duration-200">
            <td class="py-3 px-4"><?php echo htmlspecialchars($dechet['type_dechet']); ?></td>
            <td class="py-3 px-4"><?php echo htmlspecialchars($dechet['quantite_kg']); ?></td>
        
        <td class="py-3 px-4 flex space-x-2">
                        <a href="waste_edit.php?id=<?= $dechet['id'] ?>"
                        ...
                        <a href="waste_delete.php?id=<?= $dechet['id'] ?>"
